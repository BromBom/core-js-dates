...
function getDayName(date) {
  ...
  const strDay = new Intl.DateTimeFormat('en-US', options).format(objDate);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const msResult = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  const days = Math.ceil(msResult / 86_400_000) + 1;
...
function isDateInPeriod(date, period) {
  const { start: down, end: up } = period;
  return (
    new Date(date).getTime() >= new Date(down).getTime() &&
    new Date(date).getTime() <= new Date(up).getTime()
  );
}
...
function formatDate(date) {
  ...
  const min = new Date(date).getUTCMinutes().toString().padStart(2, '0');
  const sec = new Date(date).getUTCSeconds().toString().padStart(2, '0');
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (daysInMonth > 0) {
    const currentDay = new Date(year, month - 1, daysInMonth).getDay();
...
function getWeekNumberByDate(date) {
  ...
  let msStartYear = new Date(year, 0, 1).getTime();
  let weeksCounter = 1;

  do {
    msStartYear += 86_400_000;
    if (new Date(msStartYear).getDay() === 1) weeksCounter += 1;
  } while (new Date(msStartYear).getTime() < date.getTime());
...
function getNextFridayThe13th(date) {
  ...
  let friday13th = new Date(year, month, 13);

  while (friday13th.getDay() !== 5) {
    friday13th = new Date(year, month, 13);
    month += 1;
  }
